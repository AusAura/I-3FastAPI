...
@router.put('/{publication_id}/tags/add', status_code=status.HTTP_201_CREATED, response_model=TagsDetailResponse,
...
async def add_tag_to_publication(publication_id: int, body: TagSchema, db: AsyncSession = Depends(get_db),
    ...
    return {
        "detail": f"add tag to publication with id: {publication_id} ",
        "tag": tag
    }


@router.delete('/{publication_id}/tags/delete', response_model=TagsDetailResponse,
...
async def delete_tag_from_publication(publication_id: int, body: TagSchema, db: AsyncSession = Depends(get_db),
    ...
    return {
        "detail": f"delete tag from publication with id: {publication_id} ",
        "tag": body
    }
